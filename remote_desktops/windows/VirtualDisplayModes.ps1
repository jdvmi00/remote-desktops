...
try {
  ...
  $present = @($entries | Where-Object { [int]$_.width -eq $Width -and [int]$_.height -eq $Height })
  ...
  if ($present.Count -eq 0) {
    ...
    # Oldest entries go first; the newest stays.
    while (@($list.SelectNodes('resolution')).Count -gt $Keep) { [void]$list.RemoveChild(@($list.SelectNodes('resolution'))[0]) }
    $tmp = "$Settings.tmp"
    $xml.Save($tmp)
