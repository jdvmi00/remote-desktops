                    ...
                    ColumnLayout {
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                ActionButton { objectName: "refitAction"; text: "Refit"; icon.source: "qrc:/qml/icons/sliders.svg"; visible: (root.connected || root.phase === "running") && !!root.selected && root.selected.refit_available === true; enabled: !!root.selected && !!root.selected.refit_available && !root.selected.busy && manager.available && !root.transitioning; hint: root.selected && root.selected.refit_available ? "Request the window size from Sunshine; host resolution may remain unchanged" : root.selected && root.selected.platform === "windows" ? "Choose Sunshine matching, or complete SSH verification, to use Refit" : "This host display mode does not support Refit; resizing scales the picture"; onClicked: manager.act(root.selected.computer, "refit") }
                                Item { Layout.fillWidth: true }
